  ...
  return (
    <div className="space-y-6">
        ...
        <StaggerItem>
          <Card className="border-0 shadow-lg rounded-2xl card-hover">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Orçamento Restante</p>
                  <p className={`text-2xl font-bold mt-1 ${statusColor}`}>
                    {budget.total > 0 ? formatCurrency(remaining) : "Não definido"}
                  </p>
                </div>
                <div className={`rounded-2xl p-3.5 ${overBudget ? "bg-destructive/10" : nearBudget ? "bg-[hsl(var(--warning))]/10" : "bg-[hsl(var(--success))]/10"}`}>
              ...
              {budget.total > 0 && (
                <div className="mt-4">
                  ...
                  <div className="h-2.5 w-full overflow-hidden rounded-full bg-secondary">
                    <div className={`h-full rounded-full transition-all duration-700 ease-out ${progressColor}`} style={{ width: `${Math.min(percentUsed, 100)}%` }} />
                  </div>
                </div>
              )}
            </CardContent>
        ...
        <StaggerItem>
          <Card className="border-0 shadow-lg rounded-2xl card-hover">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Nº de Gastos</p>
      ...
      {(overBudget || nearBudget) && budget.total > 0 && (
        <FadeIn>
          <div className={`flex items-center gap-3 rounded-2xl p-4 ${overBudget ? "bg-destructive/10 text-destructive" : "bg-[hsl(var(--warning))]/10 text-[hsl(var(--warning))]"}`}>
